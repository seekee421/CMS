...
public class CacheOptimizationScheduler {
    ...
    private static final Logger logger = LoggerFactory.getLogger(
        CacheOptimizationScheduler.class
    );
    ...
    public CacheOptimizationScheduler(
        CacheMemoryOptimizer memoryOptimizer,
                ...
                CompletableFuture.runAsync(() -> {
                    try {
                        memoryOptimizer
                            ...
                            .thenAccept(result -> {
                                logger.info(
                                    "内存清理完成: 清理了{}个条目，释放了{}字节内存",
                                    result.getKeysRemoved(),
                                    result.getMemoryFreed()
                                );
                            });
                    } catch (Exception e) {
    ...
    public void checkCacheHealth() {
        try {
            ...
            if (!healthStatus.isHealthy()) {
                ...
                if (
                    healthStatus.getCachePerformance() != null &&
                    !healthStatus.getCachePerformance().isHealthy()
                ) {
                    issues
                        .append("缓存性能问题(命中率: ")
                        .append(healthStatus.getCachePerformance().getHitRate())
                        .append("%, 响应时间: ")
                        .append(
                            healthStatus
                                .getCachePerformance()
                ...
                if (
                    healthStatus.getCacheSize() != null &&
                    !healthStatus.getCacheSize().isHealthy()
                ) {
                    issues.append("缓存大小问题; ");
                }

                if (
                    healthStatus.getCacheConfiguration() != null &&
                    !healthStatus.getCacheConfiguration().isHealthy()
                ) {
                ...
                if (healthStatus.getErrorMessage() != null) {
                    issues
                        .append("错误信息: ")
                        .append(healthStatus.getErrorMessage());
                }
    ...
    public void performDeepOptimization() {
        try {
            ...
            // 2. 性能数据清理
            CompletableFuture<Void> performanceTask =
                CompletableFuture.runAsync(() -> {
                    try {
