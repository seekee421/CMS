...
export async function POST(req: Request) {
  try {
    ...
    const resp = await fetch(`${apiBase}/api/editor/content/save`, { method: "POST", headers, body: JSON.stringify(body ?? {}) });
    ...
    return NextResponse.json({ message: data?.message || "保存失败" }, { status: resp.status || 500 });
  } catch (e) {
