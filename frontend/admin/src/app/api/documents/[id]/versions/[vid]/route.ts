...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string; vid: string }> }) {
  try {
    const { id, vid } = await context.params;
    const apiBase = getApiBase();
    const authHeader = await buildAuthHeader();
    const headers: HeadersInit = { Accept: "application/json", ...authHeader };
    const resp = await fetch(`${apiBase}/api/documents/${id}/versions/${vid}`, { method: "GET", headers, next: { revalidate: 0 } });
