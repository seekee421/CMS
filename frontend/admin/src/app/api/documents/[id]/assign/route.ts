...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const resp = await fetch(`${apiBase}/api/documents/${id}/assign`, { method: "POST", headers, body });
    const data = await resp.json().catch(() => null);
    if (resp.ok) return NextResponse.json(data ?? { success: true }, { status: 200 });
    if (resp.status === 403) {
      return NextResponse.json(
        { message: data?.message || "权限不足：需要 DOC:ASSIGN。请联系管理员为你的角色分配该权限。", requiredPermission: "DOC:ASSIGN", code: 403 },
        { status: 403 }
      );
    }
    return NextResponse.json({ message: data?.message || "分配失败" }, { status: resp.status || 500 });
  } catch (e) {
